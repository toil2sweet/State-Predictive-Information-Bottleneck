...
- Shared training entry points stay at the repository root, including
  `test_model.py` and `test_model_advanced.py`.
- Shared experiment configurations stay under `examples/`.
- Four-Well-specific plotting utilities are under `four-well_SPIB/`; its
  trajectory generator and generated arrays are kept together under `traj_gen/`.
- Server helpers and server-specific documentation are grouped under `nscc/`,
  `hku/`, and `dna/`. The NSCC guide is `nscc/NSCC_WORKFLOW.md`.
